...
export function ParticleText({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      const padding = Math.max(8, Math.ceil(resolvedSize * 0.06));
      ...
      // Calculate horizontal positioning based on align prop
      let baseOffsetLeft = 0;
      if (align === 'center') {
        baseOffsetLeft = width / 2 - offscreen.width / 2;
      } else if (align === 'right') {
        baseOffsetLeft = width - offscreen.width;
      } else {
        // left aligned: eliminate left padding offset
        baseOffsetLeft = -padding + left;
      }

      const baseOffsetTop = height / 2 - offscreen.height / 2;
      ...
      const maxParticles = Math.max(900, Math.min(5200, Math.floor((width * height) / 80)));
